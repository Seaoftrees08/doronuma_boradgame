...
| カード名 | 枚数 | 効果 |
|:---|:---:|:---|
| 1点獲得 | 5 | 自分の手札に +1点カードを1枚加える |
| 2点獲得 | 3 | 自分の手札に +1点カードを2枚加える |
| 3点獲得 | 2 | 自分の手札に +1点カードを3枚加える |
| 単なる嫌がらせ | 8 | 指定した1人に -3点カードを1枚引かせる |
| 道連れ | 4 | 自分以外の全員に -3点カードを1枚ずつ引かせる |
| 集中砲火 | 4 | 指定した1人に -3点カードを2枚引かせる |
| 泥沼引き込み | 4 | 指定した1人に -3点カードを1枚引かせ、さらに自分以外の全員に -3点カードを1枚ずつ引かせる |
| 完全無効 | 2 | 相手の妨害系カードの効果を完全に打ち消す（自分への攻撃のみ無効化） |
| なすりつけ | 2 | 自分への妨害系カードの効果を、指定した別の人にそらす |
| 倍返し | 1 | -3点カードを引かされそうになった時、攻撃者自身に効果を二倍にして返す |
| はじき返し | 5 | 自分に向けられた妨害系カードの効果を攻撃者本人に返す |
| 強奪 | 3 | 相手1人を指定し、その人が手持ちの勝利点カードを2枚ランダムに奪う |
| 手札狙い | 2 | 相手1人を指定し、その人の行動カードの手札から1枚奪う |
| 出る杭を打つ | 2 | +5点カードを持っているプレイヤーは全員、+5点カードを1枚捨てる |
| お裾分け | 2 | 自分の任意の勝利点カードを1枚、指定した相手に押し付ける |
| 突然死 | 1 | 引いた本人が -3点カードを1枚受け取り、そのラウンド終了時にゲーム終了 |
...
共有パッケージ・バックエンドサーバー・フロントエンドコンポーネントに対するユニットテストおよび結合テストには [Vitest](https://vitest.dev/) を使用しています。

```bash
# 全テストを一括実行（シングルラン）
npx vitest run

# 開発中の変更自動検知モード（ウォッチモード）
npx vitest
```
